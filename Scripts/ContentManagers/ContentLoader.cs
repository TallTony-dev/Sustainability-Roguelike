...
namespace Monogame_Cross_Platform.Scripts.ContentManagers
{
    ...
    {
        public static (Texture2D, Rectangle)[] texturesLoaded = new (Texture2D, Rectangle)[16]; //Change this array size to the max number of textures and allocate different ranges for different textures
        ...
        public void LoadTextures(string contentListToLoad)
        {
            if (contentListToLoad == "AlwaysLoaded")
                foreach ((string, Rectangle, ushort) texture in contentList.GetAlwaysLoaded())
                {
                    texturesLoaded[texture.Item3] = (alwaysLoadedcontentManager.Load<Texture2D>("C:/Users/User/source/repos/Monogame Cross Platform/bin/Debug/net6.0/Content/Graphics/" + texture.Item1), texture.Item2);
                }
            else if (contentListToLoad == "HomeTextures")
                foreach ((string, Rectangle, ushort) texture in contentList.GetHomeTextures())
                {
                    texturesLoaded[texture.Item3] = (game.Content.Load<Texture2D>("C:/Users/User/source/repos/Monogame Cross Platform/bin/Debug/net6.0/Content/Graphics/" + texture.Item1), texture.Item2);
                }
        ...
        /// </summary>
        public static (Texture2D, Rectangle) GetLoadedTexture(ushort index)
        {
            return texturesLoaded[index];
        }
        public static Rectangle GetTextureRectangle(ushort index)
        {
            return texturesLoaded[index].Item2;
        }
